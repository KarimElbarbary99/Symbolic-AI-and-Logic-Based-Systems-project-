...
xml_file_path = "C:\\Users\\PC\\Downloads\\example-problems-mini.xml"  # Edit this path to point to your XML file containing the problems
rdf_path = "C:/Users/PC/Desktop/mini_rdf.rdf"  # Edit this path to point to the your RDF file
solution_xml_path = "solutions_mini.xml"  # Edit this path to point to the resulting XML file containing the solutions
...
def transform_xml(input_xml, query_in,id,type):
    ...
    for idx, result in enumerate(root.findall(".//xmlns:result", namespaces=name_space)):
        ...
        query.text = query_in
        if type == 'coauthors':
        # Extract and add Author elements
...
def load_tripls(file_path):

    sparql_endpoint = "http://localhost:9999/blazegraph/namespace/kb/sparql"

    # Set the SPARQL query for loading RDF triples
...
def get_query_coauthors(author_id):
    ...
    WHERE {
    ?document ns1:author_id <https://zbmath.org/authors/?q=ai%3Azhu.yucan> .
    ?document ns1:author_id ?coauthor .
